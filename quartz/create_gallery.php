	...
	<?php 
		if(isset($_GET['action']) && $_GET['action'] == 'create' && isset($_POST['gallery_name']) && isset($_POST['event_date']))
		{
			...
			else
			{
				do
				{
					$gallery_id = str_pad(mt_rand(0,999999999), 9, "0");
					$res = mysql_query("SELECT * FROM galleries WHERE gallery_id = '" . $gallery_id . "'");
			...
			$res = mysql_query("INSERT INTO galleries VALUES ('" . $gallery_id . "', '" . $gallery_name . "', '" . $event_date . "', NOW())");
			if($res !== false)
			{
				mkdir('galleries/' . $gallery_id, 0700)
				header("Location: edit_gallery.php?g=" . $gallery_id);
				exit;
			}
			else
			{
